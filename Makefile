CXX      := g++-5
CXXFLAGS := -fopenmp -pthread -fno-strict-aliasing -std=c++1y -pedantic -Wall -I../third/aquila `pkg-config --cflags opencv libpng sndfile samplerate`
LDFLAGS  := -fopenmp -L/opt/local/lib -L../third/aquila/ -L../third/aquila/lib
LIBS     := -lm -lpthread -lX11 -lpthread -lAquila -lOoura_fft -lboost_program_options `pkg-config --libs opencv libpng sndfile samplerate`
